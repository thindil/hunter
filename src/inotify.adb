...
package body Inotify is
   ...
   overriding procedure Initialize(Object: in out Instance) is
      function Inotify_Init return GNAT.OS_Lib.File_Descriptor with
         Import,
         Convention => C,
   ...
   function Add_Watch
     (Object: in out Instance; Path: String; Mask: Watch_Bits := All_Events)
      return Watch is
      function Inotify_Add_Watch
        (Instance: GNAT.OS_Lib.File_Descriptor; Path: Interfaces.C.char_array;
         Mask: Interfaces.C.unsigned) return Interfaces.C.int with
         Import,
         Convention => C,
         External_Name => "inotify_add_watch";

      function Convert is new Ada.Unchecked_Conversion(Source => Watch_Bits,
   ...
   procedure Add_Watch
     (Object: in out Instance; Path: String; Mask: Watch_Bits := All_Events) is
   ...
   procedure Remove_Watch(Object: in out Instance; Subject: Watch) is
      function Inotify_Remove_Watch
        (Instance: GNAT.OS_Lib.File_Descriptor; Watch: Interfaces.C.int)
...
-----------------------------------------------------------------------------
   ...
   package Move_Vectors is new Ada.Containers.Bounded_Vectors
     (Index_Type => Positive, Element_Type => Cookie_Move_Pair);
   ...
   procedure Process_Events
     (Object: in out Instance;
      ...
      Buffer: Stream_Element_Array(1 .. 4096) with
         Alignment => 4;
      ...
      function Find_Move
        (Cookie: Interfaces.C.unsigned) return Move_Vectors.Cursor is
                     ...
                     if Mask.Ignored then
                        ...
                        declare
                           Directory: constant String :=
                             Object.Watches.Element(Event.Watch);
                        begin
                           if Name_Length > 0 then
                              declare
                                 subtype Name_Array is
                                   Interfaces.C.char_array
                                     (1 .. Interfaces.C.size_t(Event.Length));
                                 subtype Name_Buffer is
                                   Stream_Element_Array(1 .. Name_Length);

                                 function Convert is new Ada
                                    ...
                                    when Moved_To =>
                                       declare
                                          Cursor: Move_Vectors.Cursor :=
                                            Find_Move(Event.Cookie);
                                          use type Move_Vectors.Cursor;
                                       begin
                                          if Cursor /=
                                            Move_Vectors.No_Element then
                                             --  It's a rename
                                             Move_Handle
                                               (Subject =>
                                                  (Watch => Event.Watch),
